...
Simplified version without cross-antenna features, band statistics, ensemble, mixup, label smoothing, warmup.
"""
...
def main():
    ...
    abstract_text = (
        ...
        "amplitude and phase representations and removes inactive guard band subcarriers. "
        "The final model achieves 96.48% validation accuracy on the localization task."
    )
    ...
    add_bullet_point(doc, "A feature engineering pipeline that transforms raw CSI data into amplitude and phase representations")
    add_bullet_point(doc, "A residual neural network architecture designed for CSI classification")
    add_bullet_point(doc, "Comprehensive evaluation comparing Random Forest and neural network approaches")
    ...
    add_figure_placeholder(doc, "Fig. 1. Feature engineering pipeline transforming raw CSI to engineered features.")

    add_body_text_no_indent(doc,
        "Our feature engineering pipeline transforms the raw CSI features through the "
        "following steps:"
    )
    ...
    add_body_text(doc,
        ...
        "timing information and provides a more meaningful representation for the "
        "classification task."
    )
    ...
    add_body_text(doc,
        ...
        "subcarriers per antenna, resulting in 104 amplitude features and 104 phase "
        "features for a total of 208 engineered CSI features."
    )

    add_body_text(doc,
        "The final feature vector combines the 5 metadata features with 208 engineered "
        "CSI features, giving a total of 213 input features for the neural network."
    )
    ...
    add_body_text_no_indent(doc,
        ...
        "is: Input (213) -> Linear(1024) -> BatchNorm -> ReLU -> Dropout(0.4) -> ResBlock "
        "(1024->512) -> ResBlock (512->256) -> ResBlock (256->128) -> "
        "Linear(64) -> BatchNorm -> ReLU -> Dropout(0.2) -> Output(10)."
    )
    ...
    add_body_text_no_indent(doc,
        "We searched multiple hyperparameter configurations, varying learning rate, "
        "weight decay, and dropout rates. Table I shows the top configurations."
    )

    # Table I: Hyperparameter configurations
    headers = ["Rank", "LR", "Weight Decay", "Dropout", "Val Acc"]
    data = [
        ["1", "0.001", "1e-5", "0.4", "96.48%"],
        ["2", "0.001", "5e-6", "0.35", "96.28%"],
        ["3", "0.0008", "1e-5", "0.4", "96.12%"],
        ["4", "0.001", "2e-5", "0.45", "96.07%"],
        ["5", "0.0012", "1e-5", "0.35", "95.97%"],
    ]
    ...
    add_body_text_no_indent(doc,
        "All models are trained with AdamW optimizer, batch size 256, maximum 200 epochs, "
        "early stopping patience of 40 epochs, and gradient clipping with max norm 1.0. "
        "We use standard cross-entropy loss for training."
    )
    ...
    add_body_text_no_indent(doc,
        ...
        "The amplitude and phase representation provides the most benefit, as it directly "
        "encodes signal strength and timing information relevant to device localization. "
        "Removing inactive subcarriers reduces noise and improves model focus on "
        "informative frequency components."
    )

    add_subsection_heading(doc, "C. Best Hyperparameters")

    add_body_text_no_indent(doc,
        "The best model uses: learning rate 0.001, weight decay 1e-5, and dropout base "
        "0.4. The residual architecture with skip connections allows for effective "
        "training of deeper networks without gradient degradation."
    )
    ...
    add_body_text_no_indent(doc,
        ...
        "by converting I/Q values to amplitude and phase representations; (2) Removing "
        "inactive guard band subcarriers reduces noise in the input data; (3) Residual "
        "networks outperform feedforward networks due to better gradient flow."
    )

    add_body_text(doc,
        "Our final model achieves 96.48% validation accuracy on the 10-class "
        "localization task, representing a 0.77% improvement over the baseline. "
        "Future work could explore convolutional architectures that process "
        "subcarriers sequentially or attention mechanisms to weight important frequencies."
    )
    ...
    refs = [
        ...
        "[3] IEEE 802.11bf Task Group, \"WLAN Sensing,\" IEEE Standards Association, 2024.",
    ]
    ...
    # Save document
    doc.save('IEEE_Report_WiFi_Localization_Simple.docx')
    print("Report saved to 'IEEE_Report_WiFi_Localization_Simple.docx'")
